...
class PrefixTuningTemplate(Template):
    ...
    def get_past_key_values(self, batch_size=1):
        ...
        if self.config.is_encoder_decoder and self.using_encoder_past_key_values:
            ...
            past_key_values = self.layers(self.param).unsqueeze(0).expand(batch_size,-1,-1,-1)
            seqlen = self.num_token
    ...
    def generate_parameters(self) -> None:
        ...
        lis_k,lis_v=[],[]
        for keys in data:
            if 'k' in keys: lis_k.append(torch.tensor(data[keys]).t()[:self.num_token,:].unsqueeze(1))
            else: lis_v.append(torch.tensor(data[keys]).t()[:self.num_token,:].unsqueeze(1))
        
        lis_k=torch.cat(lis_k,dim=1)
        lis_v=torch.cat(lis_v,dim=1)
        #self.param = nn.Parameter(torch.cat([lis_k,lis_v],dim=1))
        self.param = nn.Parameter(nn.init.normal_(torch.empty(self.num_token,self.match_n_layer*2,self.n_embd)))
